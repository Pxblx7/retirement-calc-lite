...
import { type SeoComparisonTemplate } from "@/lib/seo-comparisons"

// No toggles here anymore
// ─── Core Component ─────────────────────────────────────────────────────────────

export interface SimulatorCoreProps {
  seoTemplate?: SeoComparisonTemplate
}

export function SimulatorCore({ seoTemplate }: SimulatorCoreProps) {
  // Merge default config with possible SEO initial config safely preserving nested objects
  const [config, setConfig] = useState<SimConfig>(() => {
    const base = getDefaultConfig()
    const seo = seoTemplate?.initialConfig || {}
    return {
      ...base,
      ...seo,
      afore: { ...base.afore, ...(seo.afore || {}) },
      ppr: { ...base.ppr, ...(seo.ppr || {}) },
      private: { ...base.private, ...(seo.private || {}) },
    }
  })
  ...
  return (
    <div className="min-h-screen bg-background text-foreground selection:bg-primary/20 flex flex-col">
        ...
        {seoTemplate && (
          <div className="mb-8 text-center max-w-3xl mx-auto space-y-4">
            <h1 className="text-3xl lg:text-4xl font-extrabold tracking-tight text-foreground">
              {locale === 'en' ? seoTemplate.enH1 : seoTemplate.h1}
            </h1>
            <p className="text-lg text-muted-foreground">
              {locale === 'en' ? seoTemplate.enSubtitle : seoTemplate.subtitle}
            </p>
          </div>
        )}
      ...
      {/* FAQ Section */}
      <div className="mx-auto max-w-[1440px] px-4 w-full mb-16">
        <FaqSection />
      </div>
...
// Removed old SimulatorPage export directly since this is now SimulatorCore
